...
class RKIMEX:
    """
    A class to implement IMEX Runge-Kutta time-stepping (a very simplified version of what dedalus uses)
    Applies to equations of the form U_t = G(u) + F(u)
    where F is nonlinear & doesn't depend on time. G is assumed to be linear and diagonal.
    """

    # Butcher tableau for the explicit scheme
    A = NotImplemented
    # the Butcher tableau for the implicit scheme
    H = NotImplemented
    ...
    def step(self, u, h, f, g):
        ...
        Parameters
        ----------
        u - (np.ndarray) the current state
        h - (float) the step size
        f - (func) the nonlinear term
        g - (np.ndarray) a representation of the linear term (assumed diagonal)
        """
...
class RK664(RKIMEX):
    ...
    Additive Runge–Kutta schemes for convection–diffusion–reaction equations (2003)

    But really: https://scicomp.stackexchange.com/questions/21866/fourth-order-imex-runge-kutta-method
    """
    ...
    b = H[-1]
    b_hat = np.array([4586570599/29645900160, 0, 178811875/945068544, 814220225/1159782912, -3700637/11593932, 61727/225920])
    have_b = True
